...
namespace prjRentalManagement.Controllers
{
    public class AppointmentController : Controller
    {
        ...
        {
            var model = new appointment { appointmentDate = DateTime.Today };

            if (Session["manager"] != null)
            {
                if (!db.tenants.Any())
                {
                    TempData["Error"] = "No tenants are registered yet. A tenant must exist before you can create an appointment.";
            ...
            if (Session["tenant"] != null)
            {
                if (!db.managers.Any())
                {
                    TempData["Error"] = "No managers are available. Please contact support.";
                    return RedirectToAction("Index");
                }
        ...
        public ActionResult Create([Bind(Include = "appointmentId,managerId,tenantId,appointmentDate,description")] appointment appointment)
        {
            ...
            // HTML5 date posts as date-only; reject SQL Server datetime out-of-range defaults
            if (appointment.appointmentDate.Year < 1753)
                ModelState.AddModelError("appointmentDate", "Please choose a valid appointment date.");
