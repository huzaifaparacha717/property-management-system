...
namespace prjRentalManagement.Controllers
{
    public class TenantController : Controller
    {
        ...
        // GET: Tenant — manager: all tenants; tenant: own profile only
        public ActionResult Index(string searchQuery)
        {
            ...
            var tenantId = Convert.ToInt32(Session["tenant"]);
            return View(db.tenants.Where(t => t.tenantId == tenantId).ToList());
        }
        ...
        // GET: Tenant/Create — managers only (admin)
        public ActionResult Create()
        {
            if (Session["manager"] == null)
                return RedirectToAction("Register", "TenantAccess");
            return View();
        }
        ...
        {
            // Manager ya logged-in tenant
            if (Session["manager"] == null && Session["tenant"] == null)
                return RedirectToAction("Index", "Home");

            // Tenants may only edit their own profile; managers may edit any
            if (Session["tenant"] != null && (int)Session["tenant"] != tenant.tenantId)
                return new HttpStatusCodeResult(HttpStatusCode.Forbidden);
        ...
        public ActionResult Delete(int? id)
        {
            ...
            // Tenants see delete for own account; managers for any tenant
            if (Session["manager"] != null)
                return View(tenant);
            if (Session["tenant"] != null && (int)Session["tenant"] != tenant.tenantId)
                return new HttpStatusCodeResult(HttpStatusCode.Forbidden);
